-- Follow-up to 40_task_review_verdict.sql: a Revisor must never review a task they claimed and
-- did themselves - conflict of interest, regardless of role (an Operator/Coordinator/Admin who
-- also holds the REVISOR qualification is still excluded from their own work). Fixed in both
-- places: the queue never shows it to them, and the verdict RPC refuses it too (defense in
-- depth, in case something ever calls it directly instead of through the queue).
-- Do after 40_task_review_verdict.sql.
...
select 'no-self-review guard installato.' as result;
