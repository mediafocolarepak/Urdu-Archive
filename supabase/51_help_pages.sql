-- 51_help_pages.sql
-- In-app, database-backed help/documentation pages (replaces the external Claude Artifact links).
-- Admin can edit and save these directly from the app (raw HTML source editor).

create table if not exists help_pages (
  slug text primary key,
  language text not null check (language in ('it','en')),
  role text check (role in ('user','operator','coordinator','admin')),
  title text not null,
...
drop policy if exists help_pages_select on help_pages;
create policy help_pages_select on help_pages
  for select using (current_role_is('user'));

drop policy if exists help_pages_admin_insert on help_pages;
create policy help_pages_admin_insert on help_pages
  for insert with check (current_role_is('admin'));

drop policy if exists help_pages_admin_update on help_pages;
create policy help_pages_admin_update on help_pages
  for update using (current_role_is('admin')) with check (current_role_is('admin'));

drop policy if exists help_pages_admin_delete on help_pages;
create policy help_pages_admin_delete on help_pages
